<a href="/products/{{ $product['slug'] }}" class="product-card group block">
    <div class="product-card-media">
        <img src="{{ $product['images'][0] ?? '/images/products/hero-poster.jpg' }}" alt="{{ $product['name'] }}" class="h-full w-full object-cover transition duration-700 group-hover:scale-105">
    ...
    <div class="mt-4 flex items-start justify-between gap-4 px-1">
        <div class="min-w-0">
            ...
            <p class="product-card-meta">{{ $product['categoryName'] ?? Str::headline($product['category'] ?? 'Products') }}</p>
